...
/** Internal app: only TreeNewal and its agency partner may sign up. */
const ALLOWED_DOMAINS = ["treenewal.com", "thinkcre8tive.com", "test.local"];

function assertAllowedDomain(email: string) {
  const domain = email.split("@")[1]?.toLowerCase() ?? "";
  if (!ALLOWED_DOMAINS.includes(domain)) {
    throw new Error("Only TreeNewal email addresses are allowed");
  }
}
