...
export async function signInTestUser(page: Page): Promise<void> {
  ...
  if (hasEmailForm) {
    ...
    // Apps whose auth.providers excludes email_password have no login form.
    // Preview and local builds still show the test-user button, which signs
    // in through the test provider and creates the account on first use.
    const testUserButton = page.getByRole("button", {
      name: "Continue as Test User",
    });
    const hasTestUserButton = await testUserButton
      .isVisible()
      .catch(() => false);
    if (!hasTestUserButton) {
      throw new Error(
        "No email/password form and no test-user button on /login — with " +
          "email + password disabled, automated sign-in works only against " +
          "a local server or a preview build (test provider registered via " +
          "VIKTOR_SPACES_IS_PREVIEW=true)",
      );
    }
    console.log(
      "[Auth] No email form (email_password provider disabled), using the test-user button...",
    );
